...
def motifcaps(seq, idxs, motif):
    ...
    """
    Capitalizes all motif appearances in seq and returns string with capped
    motif substrings.
...
def lnsep(length):
    ...
    """
    Creates a line separator of given length.
...
def outwrapper(idxs, header, seq_capped):
    ...
    seq_lenght = f'sequence length:{len(seq_capped):24}\n'
    ...
    wrapped_seq = ''
    prev_length = 0
    for i in range(len(seq_capped) // header_length):
        wrapped_seq += seq_capped[prev_length:header_length + prev_length] + '\n'
        prev_length += header_length
    wrapped_seq += seq_capped[prev_length:] + '\n'
